---
name: gum
description: "Use when a Node.js backend needs @steamory-agent-kit/gum for agent or application Memory, including conversation sessions, contextual recall, message storage, or user action logging. Do not use for browser-only frontend code that would expose Gum API keys."
---
...
5. Initialize a Session with `gum.sessions.init(...)`: pass `{ user_id, session_id, title, metadata }` for a new conversation, or pass a stored Session id string to restore the local object-style API without a network request.
...
9. Run the host project's typecheck, formatter/linter, and build commands after integration.
10. Handle `GumApiError`, `GumConnectionError`, and `GumTimeoutError` explicitly in backend boundaries.
...
- Use `gum.sessions.init({ user_id, session_id, title, metadata })` for new conversations.
- Store `session.id` in the application database when future turns must continue the same memory thread.
- Use `await gum.sessions.init(sessionId)` to recreate the local `Session` object without a network request.
...
- In existing chat apps, prefer the app's stable `chatId`/conversation id as Gum `session_id` and the authenticated account id as `user_id`.
- For an existing chat, restore with `await gum.sessions.init(chatId)` instead of calling the create form again.
- Keep Gum code in a server-only adapter module and call that adapter from chat routes or server actions.
- If a chat app creates its database chat record only after the model response, create or reserve the stable chat id before the model call so Gum can retrieve memory for the first turn.
- If a chat app stores sessions only in browser state or local storage, do not import the Gum SDK there; add a backend Gum route or server action and call it from the client.
...
- Preserve the app's existing system/developer prompts. Append Gum memory as an extra context block before the latest user turn; do not replace product, workspace, or tool instructions.
...
- For framework apps, also run their formatter/linter and production build; TypeScript success alone may miss formatting or route bundling issues.

## References

For detailed request shapes, examples, and edge cases, read `references/sdk-usage.md`.

For existing Next.js, Vercel AI SDK, or route-based chat apps, read `references/chat-app-integration.md` before editing.

For browser-local chat stores, LLM proxy routes, or apps without backend auth, read `references/client-proxy-chat-integration.md` before editing.
